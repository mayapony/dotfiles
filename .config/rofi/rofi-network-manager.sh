...
RASI_FILE="colors.rasi"
function initialization() {
	source "$DIR/rofi-network-manager.conf" || source "${XDG_CONFIG_HOME:-$HOME/.config}/rofi/rofi-network-manager.conf"
	{ [[ -f "$DIR/$RASI_FILE" ]] && RASI_DIR="$DIR/$RASI_FILE"; } || { [[ -f "${XDG_CONFIG_HOME:-$HOME/.config}/rofi/$RASI_FILE" ]] && RASI_DIR="${XDG_CONFIG_HOME:-$HOME/.config}/rofi/$RASI_FILE"; } || exit
