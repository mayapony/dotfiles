(function (global, factory) {
	...
	(global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.UPNG = {}, global.pako));
	...
	function getNearest(nd, r,g,b,a)
	{
		...
		var planeDst$1 = planeDst(nd.est,r,g,b,a);
		
		var node0 = nd.left, node1 = nd.right;
		if(planeDst$1>0) {  node0=nd.right;  node1=nd.left;  }
		
		var ln = getNearest(node0, r,g,b,a);
		if(ln.tdst<=planeDst$1*planeDst$1) return ln;
	...
	function main(nimg, w, h, dels, tabs) {
		if(tabs==null) tabs={};
		var wUi = writeUint, wUs = writeUshort, wAs = writeASCII;
	...
	function filterZero$1(data, out, off, w, h)
	{
	...
	function readInterlace(data, out)
	{
		...
		while(pass<7)
		{
			...
			filterZero$1(data, out, di, sw, sh);
	...
	function inflate(data, buff) {  var out=pako.inflateRaw(new Uint8Array(data.buffer, 2,data.length-6),buff);  return out;  }

	function decompress(out, dd, w, h) {
		var bpp = getBPP(out), bpl = Math.ceil(w*bpp/8), buff = new Uint8Array((bpl+1+out.interlace)*h);
		if(out.tabs["CgBI"]) dd = pako.inflateRaw(dd,buff);
		else                 dd = inflate(dd,buff);
		if     (out.interlace==0) dd = filterZero$1(dd, out, 0, w, h);
	...
	{
		var data = new Uint8Array(buff), offset = 8, rUs = readUshort, rUi = readUint;
		var out = {tabs:{}, frames:[]};
		...
		while(offset<data.length)
		{
			...
			if     (type=="IHDR")  {  IHDR(data, offset, out);  }
				...
				out.tabs[type] = [];
				for(var i=0; i<8; i++) out.tabs[type].push(readUint(data, offset+i*4));
			}
			else if(type=="tEXt" || type=="zTXt") {
				if(out.tabs[type]==null) out.tabs[type] = {};
				var nz = nextZero(data, offset);
				var keyw = readASCII(data, offset, nz-offset);
				var text, tl=offset+len-nz-1;
				if(type=="tEXt") text = readASCII(data, nz+1, tl);
				else {
				...
				var keyw = readASCII(data, off, nz-off);  off = nz + 1;
				var cflag = data[off];  off+=2;
				nz = nextZero(data, off);
				var ltag = readASCII(data, off, nz-off);  off = nz + 1;
				nz = nextZero(data, off);
				var tkeyw = readUTF8(data, off, nz-off);  off = nz + 1;
				...
				if     (out.ctype==3) out.tabs[type] = readBytes(data, offset, len);
				else if(out.ctype==0) out.tabs[type] = rUs(data, offset);
				else if(out.ctype==2) out.tabs[type] = [ rUs(data,offset),rUs(data,offset+2),rUs(data,offset+4) ];
				//else console.log("tRNS for unsupported color type",out.ctype, len);
			}
			else if(type=="gAMA") out.tabs[type] = readUint(data, offset)/100000;
			...
			offset += len;
			var crc = readUint(data, offset);  offset += 4;
		}
		if(foff!=0) {  var fr = out.frames[out.frames.length-1];
			fr.data = decompress(out, fd.slice(0,foff), fr.rect.width, fr.rect.height);  foff=0;
		}
